...
function [l] = getLinkageClasses( complexCom )

% Function to generate the number of linkage classes in a reaction network
% (and which complexes are in each of them). 
% ------------------------------------------------------------------------
% Output: - Com: Output that provides the intersection between from and to
%                 matrices. Elements of Com are 1 if conditions of from or 
%                 to are true. ie complexes communicate in at least one
%                 direction.

% Input: - complexCom: Matrix that represents the communication between
%                      different complexes in the system. The format of 
%                      ComplexCom is similar to a standard transition 
%                      probability matrix. But now, consider each state to
%                      be a different complex. And also, if one complex 
%                      communicates with another, the entry for that term is 1. 
...
% And creates matrix that gives communicative complexes (described above)
Com = to|from;

% For each set of equal rows in Com, we have one linkage class
% For identifying these linkage classes:

[~, indexRep, ~] = unique(Com, 'rows');
